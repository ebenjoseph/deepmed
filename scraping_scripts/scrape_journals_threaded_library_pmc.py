...
##												 ##
outfile = open('scrape_output_pmc.jsonl', 'a')

journals_to_scrape = []
# read in csv
with open('pubmed_harvest_gt_output_cochrane_pmc.csv', 'rU') as csvfile:
...
logging.basicConfig(filename='scraping_log_pmc.log',level=logging.INFO, format='%(asctime)s [%(levelname)s] %(message)s', datefmt='%m/%d/%Y %I:%M:%S %p')
...
def check_objtype(sibs):
	try:
		...
		strong = ''
	if 'reference-list' in check_attrib(sibs,'id'):
	...
	#	#found a table
	#	objtype = "table"
	elif '__p' in check_attrib(sibs,'id'):
		#found a paragraph
		objtype = "data"
	...
	try:
		...
		for sibs in main:
			...
			if objtype == 'references':
				...
				ref = []
				#bibs = sibs.find_all('div',id=lambda x: x and (x.startswith('r') or (x.startswith('B'))))
				bibs = sibs.find_all('div',class_=lambda x: x and (x.startswith('ref-cit-blk') or (x.startswith('ref-cit-blk half_rhythm'))))
				for item in bibs:
					if not grab_text(item) == '':
						ref = ref + [grab_text(item)]
				bibs = sibs.find_all('id',id=lambda x: x and (x.startswith('B') or (x.startswith('ref-cit-blk'))))
				for item in bibs:
					if not grab_text(item) == '':
						ref = ref + [grab_text(item)]
				continue
