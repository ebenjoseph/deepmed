...
def check_time(timer):
	if time.time() - timer > 600:
		logging.info('Script has run for %s seconds', time.time()-timer)
		logging.info('Taking a break for 120 seconds')
		sleep(120)
		logging.info('Resetting timer and resuming script...')
		return time.time()
	else:
		return timer
...
def getlink(reportLink):
	...
	data['search_term'] = search_terms[termnum]
...
for library in libraries:
	...
	f = open(filepath,'w')
	f.close()
	trackerpath = library + "progress_tracker.csv"
	try:
		f = open(trackerpath)
		f.close()
	except:
		f = open(trackerpath,'w')
		f.close()		
	...
	#read in progress
	try:
		lines = open(trackerpath).readlines()[-1]
		lines = lines.split(",")
		lastpage = int(lines[1])
		numpages = int(lines[2])
		logging.info('Loaded lastpage: %s', lastpage)
		logging.info('Loaded numpages: %s', numpages)
		if lastpage == 'Not found':
			lastpage = 0
		if numpages == 'Not found':
			numpages = 0
		if lastpage == numpages:
			termnum = int(lines[0])
			lastpage = 0
		else:
			termnum = int(lines[0])-1
	except:
		termnum = -1
		lastpage = 0
		numpages = 0
	while termnum < len(search_terms):
		if counter_item > counter_end:
			logging.info('Done collecting results. Stopping...')
			break
		termnum += 1
	#for term in search_terms:
		
		#logging.info('LAST LINE LOADED:', lines)
		logging.info('Term num: %s. Term: %s', termnum, search_terms[termnum])
		logging.info('Last completed page number: %s', lastpage)
		time.sleep(2)

		logging.info('Searching "%s" library for "%s"', library, search_terms[termnum])
		link = 'http://www.ncbi.nlm.nih.gov/pubmed/?term="' + library + '"[Filter] AND "loattrfull text"[sb] AND English[lang] AND "Clinical Trial"[ptyp] AND "humans"[Filter]'
		#link = 'http://www.ncbi.nlm.nih.gov/pubmed/?term="' + search_terms[termnum] + '"[All Fields] AND "' + library + '"[Filter] AND "loattrfull text"[sb] AND English[lang] AND "Clinical Trial"[ptyp] AND "humans"[Filter]'
		...
		while (pagination == True):
			...
			if not currentPage == 'Not found':
				#if lastpage == currentPage
				#navigate to the right starting page if the process had run previously
				if lastpage + 1 > int(pageCount):
					logging.info('Last page already captured. Moving to next term.')
					lastpage = 0
					break
				elif not lastpage == currentPage:
					pageInput = lastpage + 1
					logging.info('Page input = %s', pageInput)
					#input this page into page selector and hit enter
					pageSelectorInput = getxpath(layer1,["//input[@id='pageno']"])
					pageSelectorInput.click()
					pageSelectorInput.send_keys(pageInput)
					pageSelectorInput.send_keys(Keys.ENTER)
					logging.info('Navigating to results not yet captured...')
					time.sleep(1)
			...
			# setup threadpool
			pool = ThreadPool(4)
			counter_item = counter_item + len(searchResults)
			...
			#write down the last completed page
			tracker = collections.OrderedDict()
			tracker['term_num'] = termnum
			tracker['result_page'] = currentPage
			logging.info('Current page written to file: %s', currentPage)
			tracker['total_pages'] = pageCount
			with open (trackerpath, mode='a') as csvfile:
				wr = csv.DictWriter(csvfile,tracker.keys(),dialect=csv.excel)
				wr.writerow(tracker)
			if counter_item > counter_end:
				logging.info('Done collecting results. Stopping...')
				break
			...
			if not (pagebutton == 'Not found'):
				...
				lastpage = 0
				pagination = False
			#check time elapsed and take a nap if needed
			t0 = check_time(t0)
