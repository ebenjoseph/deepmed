...
##												 ##
outfile = open('ES_output.jsonl', 'a')
...
##												 		##
articletracker = 'ES_article_tracker'
...
def grabtable(sibs):
	...
	for note in legendobj:
		legend = legend + " " + note.next_sibling.get_text().encode("utf-8")
	footnoteobj = sibs.find_all('dl',class_='tblFootnote')
	footnote = ""
	for note in footnoteobj:
		footnote = footnote + " " + note.get_text().encode("utf-8")
	#get table headers
	all_th = sibs.find_all('th')
	th = []
	for each in all_th:
		th.append(each.get_text().encode("utf-8"))
	#print th
	tablecontent = []
	#get all rows
		...
		for cell in cells:
			...
			if cols == 2:
				...
				if rowheader == "":
					...
					continue
				elif rowheader == cell.get_text():
					#just found rowheader, skip printing
					continue
				if not header == "":
					tablecontent.append(th[idx-1] + ", " + th[idx] + "; " + header + " - " + rowheader + ": " + cell.get_text())
					#print th[idx-1] + ", " + th[idx] + "; " + header + " - " + rowheader + ": " + cell.get_text()
				else:
					tablecontent.append(th[idx-1] + ", " + th[idx] + "; " + rowheader + ": " + cell.get_text())
				...
				if rowheader =="":
					...
					continue		
				if not header == "":
					tablecontent.append(th[idx] + "; " + header + " - " + rowheader + ": " + cell.get_text())
					#print th[idx] + "; " + header + " - " + rowheader + ": " + cell.get_text()
				else:
					tablecontent.append(th[idx] + "; " + rowheader + ": " + cell.get_text())
		...
		for each in main:
			...
			for sibs in each.next_siblings:
				...
				if objtype == 'header':
					...
					#print "  Subheader:" + grab_text(sibs)
					continue
				if objtype == 'table':
					#print "  Found a table"
					...
					main_content["Tables"] = main_content.get("Tables", []) + [table]
					continue
				if objtype == 'references':
					print "  Found the reference list"
					...
					main_content[header] = main_content.get(header,[]) + ref
					continue
				if objtype == 'data':
					...
					if subheader == '':
						...
						part["Content"] = part["Content"] + grab_text(sibs)
					...
					#or there is a subheader:
						...
						subdict[subheader] = subdict.get(subheader,"") + grab_text(sibs)
						main_content[header] = [subdict]
					continue
			try:
				main_content[header] = main_content[header] + [part["Content"]]
			except:
				main_content[header] = main_content[header] + part["Content"]
