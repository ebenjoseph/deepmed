...
POPULATION_WORDS = frozenset(["patients", "males", "females", "adults", "children"])
MAX_DIST = 10

# Common data objects
front_lemmas = lemmas[:num_begin]
tail_lemmas = lemmas[num_end:]
following_lemma = lemmas[num_end:num_end+1]
...
# Rule: patient population size is 10+
if (len(POPULATION_WORDS.intersection(following_lemma)) > 0) and (num_value => 10):
  yield num._replace(label=1, type='pos:population_10plus')

# Rule: patient population size is 100+
if (len(POPULATION_WORDS.intersection(following_lemma)) > 0) and (num_value => 100):
  yield num._replace(label=2, type='pos:population_100plus')

# Rule: patient population size is 1000+
if (len(POPULATION_WORDS.intersection(following_lemma)) > 0) and (num_value => 1000):
  yield num._replace(label=3, type='pos:population_1000plus')
